...
Known platform behaviour, a reliability issue and not a demonstrated authorization bypass: the first request served by a newly opened Data API backend connection may run with `auth.user_id()` = NULL, and the RPCs then fail closed with `auth_required` before any side effect. Only that correlation is established, not the cause inside Neon. A wrong identity has not been observed and would be a P0.
