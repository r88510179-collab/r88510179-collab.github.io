...
$('sendCode').onclick=async()=>{setBusy(true,'Sending sign-in code…');msg('');try{const {error}=await neon.auth.emailOtp.sendVerificationOtp({email:ADMIN_EMAIL,type:'sign-in'});if(error)throw error;$('otpWrap').hidden=false;$('otp').focus();msg(`Sign-in code sent to ${ADMIN_EMAIL}.`,'success')}catch(e){msg(`Could not send sign-in code: ${e.message||e}`,'error')}finally{setBusy(false)}};
$('verifyCode').onclick=async()=>{const code=$('otp').value.trim();if(!/^\d{4,10}$/.test(code)){msg('Enter the numeric code from your email.','error');return}setBusy(true,'Verifying…');msg('');try{const {error}=await neon.auth.signIn.emailOtp({email:ADMIN_EMAIL,otp:code});if(error)throw error;$('otp').value='';$('otpWrap').hidden=true;await refreshSession();if(!session)throw new Error('Authentication completed but no authorized session was created.');msg('Signed in securely.','success')}catch(e){msg(`Sign-in failed: ${e.message||e}`,'error')}finally{setBusy(false)}};
$('signOut').onclick=async()=>{await neon.auth.signOut();session=null;renderAuth()};

function setFile(file){if(busy)return;currentFile=file||null;candidate=null;$('review').hidden=true;$('publishResult').hidden=true;$('fileName').textContent=file?`${file.name} · ${(file.size/1024).toFixed(0)} KB`:'No file selected';$('parseBtn').disabled=!file}
